...
- In a targeted method, \(y_t\) denotes the @@latex:\textcolor{red}{t}@@arget
  class value, \(y_o\) the @@latex:\textcolor{red}{o}@@ther class values.  For
  example, \(y = [0.2, 0.5, 0.3]\) and \(t = 0\), then \(y_t = 0.2\) and
  \(y_o\in\{0.5, 0.3\}\).  Same for \(z\).
...
#+ATTR_LATEX: :booktabs t
#+CAPTION: The classifier accuracy.
...
1. {{{tex({\protect\tikz[baseline=0.5ex]{\protect\draw (0\,0) rectangle
   (2ex\,2ex)}})}}} (
   {{{tex({\protect\tikz[baseline=0.5ex]{\protect\draw[fill=black!10] (0\,0)
   rectangle (2ex\,2ex)}})}}}) data samples always correctly (incorrectly) w/
   and w/o adversarial training.
2. {{{tex({\protect\tikz[baseline=0.5ex]{\protect\draw[fill=green!10] (0\,0)
   rectangle (2ex\,2ex)}})}}} data samples correctly recognized with adversarial
   training only.
3. {{{tex({\protect\tikz[baseline=0.5ex]{\protect\draw[fill=red!10] (0\,0)
   rectangle (2ex\,2ex)}})}}} correctly recognized w/o training only, i.e., the
   side effect.
